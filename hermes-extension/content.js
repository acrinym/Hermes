...
(function() {
            ...
            if (defaultsBtn) {
                defaultsBtn.onclick = () => {
                    if (confirm("Are you sure you want to load default settings? This will update the text area. Save to apply them.")) {
                        ...
                        if (useCoordsCb) useCoordsCb.checked = !!defaultSettings.macro.useCoordinateFallback;
                        if (recordMouseCb) recordMouseCb.checked = !!defaultSettings.macro.recordMouseMoves;
                        if (simSlider) { simSlider.value = defaultSettings.macro.similarityThreshold; if (simValue) simValue.textContent = simSlider.value; }
    ...
    function runFormFiller(currentProfileDataToUse = profileData) {
            ...
            fields.forEach((field) => {
                ...
                if (profileKey && typeof currentProfileDataToUse[profileKey] !== 'undefined' && currentProfileDataToUse[profileKey] !== null) {
                    ...
                    debugLogs.push({
                        ...
                        details: { profileKey, value: currentProfileDataToUse[profileKey], score: match.score, reason: match.reason }
                    });
                }
            });
        });
        ...
        if (learningMode && skippedFields.length > 0) {
            console.log('Hermes CS: Skipped fields for training:', skippedFields.map(sf => ({label:sf.label, guess:sf.currentGuess, score: sf.score, reason: sf.reason}))); 
    ...
    function startRecording() {
        ...
        const types = ['click', 'input', 'change', 'mousedown', 'mouseup', 'keydown', 'keyup', 'focusin', 'focusout', 'submit', 'dblclick', 'contextmenu'];
        ...
        skippedFields.forEach((skipped) => {
            ...
            itemDiv.innerHTML = `
                <div style="flex-grow:1;">
                    ...
                    ${skipped.currentGuess ? `<div style="font-size:0.8em;opacity:0.7;">Initial Guess: ${skipped.currentGuess}${skipped.reason ? ` (${skipped.reason})` : ''}</div>` : ''}
                </div>
    ...
    function createHelpPanel() {
        ...
        const contentHtml = `
            ...
            <p>Hermes speeds up repetitive data entry and automates complex page interactions. It is especially helpful on sites like <strong>BMC Helix/Remedy</strong> where IDs change each load. By storing DOM paths and fallback coordinates, Hermes can reliably replay your recorded actions.</p>
            <ul style="list-style:disc;padding-left:20px;margin-bottom:15px;">
                <li>Save time by filling forms with a single click.</li>
                <li>Record macros that step through multi-stage workflows for you.</li>
                <li>Customize the UI with themes, effects and detailed settings.</li>
                <li>Debug mappings and macros with built-in logs and overlays.</li>
            </ul>
