...
export function toggleOverlays() {
  ...
  saveDataToBackground('hermes_overlay_state_ext', overlayEnabled).catch(e => console.error('Hermes CS: Failed to save overlay state', e));
  if (overlayEnabled) {
    applyOverlays();
    startObserver();
  } else {
...
export function applyOverlays() {
  const fields = document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>('input:not([type="button"]):not([type="submit"]):not([type="reset"]):not([type="hidden"]):not([type="image"]):not([type="file"]), select, textarea');
